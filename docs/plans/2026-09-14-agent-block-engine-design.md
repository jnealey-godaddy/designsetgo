...
| `designsetgo_markup_too_large` | REST `POST /agent-build/{id}/sanitize`, HTTP 413 | `markup` over 1 MB, or a body over 2 MB + 4 KB (the browser checks the markup first and reports `sanitize markup too large` without calling the route) |
