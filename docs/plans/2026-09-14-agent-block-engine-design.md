...
| `designsetgo_unknown_attribute` | PHP only (fail-open — see "Unknown attribute names" below for the exact three-part rule): a misspelled (`backgroundColour`) attribute name PHP is confident is a typo of a known one, on a block the committed manifest covers. `message` always names a suggestion: `unknown attribute "backgroundColour" for designsetgo/section — did you mean "backgroundColor"?`. The browser engine's `findUnknownAttributes()` has no such carve-out and reports this for every unknown name, suggestion or not. |
...
**Unknown attribute names.** `createBlock()` silently drops an attribute name a block type doesn't register — a misspelled (`backgroundColour`) or invented one — so the browser engine (`findUnknownAttributes()`, `src/engine/attributes.js`) rejects it in `assemble()` for **every** unknown name on **every** registered block, no exceptions: "known" there is every key `getBlockType(name).attributes` has after full registration, which already includes every block-support attribute (`style`, `className`, `anchor`, `backgroundColor`, `lock`, `metadata`, ...) and every extension-added attribute — verified empirically, so no separate allowlist exists. This is the *only* authoritative check.

PHP's up-front check (`Tree_Attributes::check()`) is a deliberately narrower, **fail-open** version of the same idea, not a full mirror. Measuring parity across every `designsetgo/*` and `core/*` block type found only 79 of 179 where PHP's own `WP_Block_Type->attributes` already covers every JS-registered name (`anchor` is the single biggest gap — WordPress's PHP registry never adds it to a block's `attributes`, only client-side block-support JS does; every DesignSetGo extension attribute is a second, larger gap, since extensions register purely in JS). A committed manifest at `includes/abilities/agent-build/data/attribute-manifest.json` (`{blockName: [attributeName, ...]}`, generated by `npm run engine -- attribute-manifest --out includes/abilities/agent-build/data/attribute-manifest.json` — see `src/engine/node/attribute-manifest.js`; kept fresh by `tests/unit/engine/attribute-manifest-freshness.test.js` and `tests/engine/cli.test.mjs`) closes most of that gap, but it only covers `designsetgo/*` and `core/*` — any other registered block (a WooCommerce block, a third-party plugin's block) isn't in it at all, and a third-party JS filter can add attributes to a `designsetgo/*`/`core/*` block that the Node-generated manifest never saw either. Rejecting in either of those cases would be a false positive: PHP has no reliable way to tell "genuinely unknown" from "known only to some JS this generator didn't run."

So PHP rejects an attribute name (`designsetgo_unknown_attribute`) only when **all three** hold: (a) the block name is itself a key in the manifest (`Attribute_Manifest::is_covered()`); (b) the name is unknown to both the block type's own PHP schema and the manifest; and (c) `Attribute_Suggest::closest()` (a PHP port of the same Damerau-Levenshtein suggestion logic as the browser engine) finds a known name close enough to plausibly be a typo. Everything else — an uncovered block entirely, or a name with no plausible suggestion — passes PHP unchecked and is judged by the browser engine at finish time, same as before this check existed. The "did you mean" text always matches whichever side catches the problem first, since both share the same distance/threshold rules.
...
| `designsetgo_sanitized_content_changed` | report `invalid` entry | The submitter lacked `unfiltered_html`, and KSES-filtering the assembled markup changed this block: its name or child count, any attribute value (the `reason` names the attributes), or its validity against its current `save()` |
| `designsetgo_markup_too_large` | REST `POST /agent-build/{id}/sanitize`, HTTP 413 | `markup` over 1 MB, or a body over 6 MB + 4 KB — room for markup the browser accepted at 1 MB even if every character JSON-escapes to a six-byte `\u00XX` (the browser checks the markup first and reports `sanitize markup too large` without calling the route, and maps this 413 to the same reason) |
| `designsetgo_build_mismatch` | REST `POST /agent-build/{id}` or `POST /agent-build/{id}/sanitize`, HTTP 409 | The request's `buildId` does not match the pending build, or nothing is pending |

On success it stores, in protected post meta registered with `auth_callback => '__return_false'` (no user holds `add_post_meta`/`edit_post_meta`/`delete_post_meta` for either key, so XML-RPC custom fields and the classic Custom Fields box cannot write them; only `Build_Store`, through `update_post_meta()`, does):

- `_dsgo_pending_tree` — one JSON blob: `{ tree, mode, base, submitter, submitterUnfiltered, buildId }`, where `base` is the post's `post_modified_gmt` when stored, `submitter` is the id of the user who called `build-page`, `submitterUnfiltered` is whether that user held `unfiltered_html`, and `buildId` is a fresh `wp_generate_uuid4()`. The blob also carries `signature`: `hash_hmac( 'sha256', wp_json_encode( { postId, submitter, submitterUnfiltered, buildId, base, mode, tree } ), wp_salt( 'auth' ) )`, computed over the values after one JSON round trip so it re-verifies exactly. `postId` is not stored in the blob; `pending( $post_id )` supplies it, so a valid blob copied onto another post does not verify there. `Build_Store::pending()` treats a blob whose signature is missing or fails `hash_equals` as nothing pending — a forged or edited blob can never claim a submitter, skip sanitizing, or change the tree. The report meta is not signed: no decision reads it. (Implementation note: `base` lives inside this same meta value rather than a separate `_dsgo_pending_base` key as an earlier draft of this doc specified — one read gives everything atomically. See `Build_Store::META_PENDING_TREE`.) Both meta values are written `wp_slash()`ed, because `update_post_meta()` unslashes and would otherwise strip the escapes `wp_json_encode()` adds for quotes, newlines, backslashes and non-ASCII.
- `_dsgo_build_report` — `{ status: "pending", buildId, treeHash }`

**KSES and the submitter.** The tree is saved later by whoever opens the editor, who may hold `unfiltered_html` when the submitter does not. So when the submitter lacks `unfiltered_html`, the build is filtered twice, each time exactly as core filters that user's own saves:

1. **Attributes, at `build-page`** (`Tree_Kses`): each node's `attributes` go through core's `filter_block_kses_value( $attributes, 'post', wp_allowed_protocols(), array( 'blockName' => $name ) )` — the filter `filter_block_kses()` applies to a block comment's attributes, including attribute keys and the `core/template-part` `tagName` check. Like core, it entity-encodes plain text (a CSS `.a > .b` is stored as `.a &gt; .b`). That is a deliberate trade-off: core does exactly this to a contributor's own comment-stored strings when they save, and any filter that kept `>` would no longer match core.
2. **Rendered markup, in the editor** (`Build_Sanitize_REST`): attribute filtering cannot see what `save()` renders — a `javascript:` URL in a URL attribute, an unsafe inline `style`. After `assemble` and before applying (in both the auto-save and review branches), the finish plugin POSTs the markup to `POST /designsetgo/v1/agent-build/{id}/sanitize` with `{ buildId, markup }` (`edit_post`; 404 missing post; 409 `designsetgo_build_mismatch`; 413 `designsetgo_markup_too_large`, checked before filtering). The route returns `{ markup: wp_kses( $markup, 'post' ) }`: core's `pre_kses` hook runs `filter_block_kses()` over the block comments, and HTML attributes get protocol and `safecss_filter_attr` filtering. The plugin parses the result and compares it with the unfiltered parse, block by block at the same tree position. It applies the **sanitized** blocks only if the structure (names and child counts, recursively) is unchanged, every attribute is deep-equal (rich-text values compared by their HTML string), and every block passes `wp.blocks.validateBlock()` against its *current* `save()` — `isValid` alone is not trusted, because the parser also marks a block valid after migrating it through a deprecation. Byte equality is deliberately not required: KSES normalizes harmless markup such as `<br/>` to `<br />`. Otherwise it reports `failed` with one `designsetgo_sanitized_content_changed` entry per changed block (naming the changed attributes) and applies nothing. A failed sanitize request, or markup over 1 MB (checked in the browser first), is `failed` too. In practice a `core/button` `url: "javascript:alert(1)"` fails because the filter rewrites the href to `alert(1)` and the `url` attribute changes; a `core/image` `data:` src fails the same way; a `designsetgo/icon-button` with a `javascript:` url fails validation, because its `url` lives in the block comment and no longer matches the filtered href. Ordinary DSGo markup comes back byte-identical and is applied.

A submitter with `unfiltered_html` has the tree stored unchanged and the sanitize route is skipped (`submitterUnfiltered === true`, strictly).
...
4. Unless `submitterUnfiltered` is `true`, sanitize the markup through the sanitize route (see **KSES and the submitter**). If the filter changed a block, or the request failed, report `failed` and change nothing; otherwise continue with the sanitized blocks.
5. If valid:
   - **Auto-save** — only when `isSubmitter` is true **and** the post is not `publish`/`future`/`private`: apply (`replace` or `append`), save through REST, report `finished` or `finished_with_findings` (which clears `_dsgo_pending_tree`). If the save fails or throws, the previous blocks are restored before `failed` is reported, so core autosave never writes a build whose report already says `failed`.
   - **Review** — every other case: a published/scheduled/private post, or a build submitted by another user. Hold automatic saves **before** applying: lock core autosave (`lockPostAutosaving( 'designsetgo-agent-build' )`) and add an `editor.preSavePost` filter (namespace `designsetgo/agent-build-review`) that throws "Save or discard the agent build before previewing." for any save with `isPreview` or `isAutosave`, then apply to the editor **unsaved**, report `awaiting_review`, and show a non-dismissible review notice with a **Discard** action ("Review agent changes before updating." for the submitter; for someone else's build, a notice that an agent submitted the changes on behalf of another user and they must be reviewed before saving). Saving reports `finished` or `finished_with_findings`; Discard restores the previous blocks, reports `discarded`, and cancels the pending save report, so a later save reports nothing. Both clear the pending meta. Leaving without either keeps the tree pending (`awaiting_review` is the only non-terminal report), so it is applied again on the next editor load. The filter is needed because the lock alone does not stop Preview: with classic meta boxes on the page, core passes `forceIsAutosaveable`, which skips the lock, and for a draft `__unstableSaveForPreview` calls `savePost( { isPreview: true } )`, which updates the post itself. `savePost()` runs the filter through `applyFiltersAsync()` before `saveEntityRecord()`; a throw aborts the save and core shows "Updating failed." with the message appended. The lock and the filter are released together on Discard and after the next successful manual save — never by an autosave or a preview save. Core's `isAutosavingPost()`/`isPreviewingPost()` read false the moment a save ends, so the save watcher samples them while the save is in flight. If applying for review throws before the Discard notice and the save watcher are both in place — including a subscriber throwing after `resetBlocks` already updated the store — the previous blocks are restored and the lock and filter released (if restoring itself throws, both stay held). Without it, core autosave on a draft the opener authored calls `wp_update_post()` on the post itself about a minute later, saving the build under the opener's capabilities with nobody clicking Save (and the next load would report a false `conflict`). `lockPostAutosaving` exists in every WordPress this plugin supports (`@wordpress/editor` 12, WP 5.9+), so there is no fallback branch.
6. Set `data-dsgo-finish="done"` or `"failed"` on the document element so a headless browser knows when to stop waiting.
...
An automatic save happens only in the submitter's own editor session, so it runs under the same capabilities that submitted the tree, and core's KSES applies to that save as usual. A build submitted by someone else is never saved automatically — not by the finish plugin and, because autosave is locked, not by core — and what the reviewer sees and may save has already been through the same attribute and markup filtering core would have applied to the submitter's own save.
...
7. **Remote flow end to end** — `build-page` over REST, open `finish_url`, then assert each outcome: `finished`, `finished_with_findings`, `failed`, `conflict`, and for a published post `awaiting_review` (left unsaved with the review notice), then `discarded` after Discard. Also: a paragraph with a link and non-ASCII text finishes with its content intact; a `core/list > core/list-item` tree is accepted and finished; a contributor's build opened by an administrator lands in `awaiting_review`, unsaved, passes through the sanitize route, and holds the autosave lock until Discard; with a classic meta box on the page, Preview during that review saves nothing and the report stays `awaiting_review`; a contributor `core/button` with a `javascript:` url, a `core/image` with a `data:` src, and a `designsetgo/icon-button` with a `javascript:` url each fail with `designsetgo_sanitized_content_changed`. PHPUnit covers each `build-page` rejection code, attribute filtering parity with core, the sanitize route (protocol/script/event-handler/CSS/template-part filtering, byte-identical ordinary markup, 403/404/409/413), `submitterUnfiltered`, the meta keys granting no meta capability (including XML-RPC custom fields), tampered or unsigned pending blobs reading as nothing pending, the `buildId` 409 cases, and terminal statuses.
