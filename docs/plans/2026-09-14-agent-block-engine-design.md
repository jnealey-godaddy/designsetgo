...
| `designsetgo_unknown_attribute` | Attribute name not in `WP_Block_Type->attributes` AND not in the committed JS-registered-attribute manifest (see below) — a misspelled (`backgroundColour`) or invented name. `message` names the attribute and, when a known name is a plausible typo away (Damerau-Levenshtein ≤ 2, or ≤ 3 for names ≥ 10 characters; case-only differences count as 0), suggests it: `unknown attribute "backgroundColour" for designsetgo/section — did you mean "backgroundColor"?`. |
...
**Unknown attribute names.** `createBlock()` silently drops an attribute name a block type doesn't register — a misspelled (`backgroundColour`) or invented one — so both the browser engine (`findUnknownAttributes()`, `src/engine/attributes.js`) and PHP's up-front check (`Tree_Attributes`) reject it instead. The browser engine is authoritative: "known" there is every key `getBlockType(name).attributes` has after full registration, which already includes every block-support attribute (`style`, `className`, `anchor`, `backgroundColor`, `lock`, `metadata`, ...) and every extension-added attribute — verified empirically, so no separate allowlist exists. PHP cannot see that: measuring parity across every `designsetgo/*` and `core/*` block type found only 79 of 179 where PHP's own `WP_Block_Type->attributes` already covers every JS-registered name (`anchor` is the single biggest gap — WordPress's PHP registry never adds it to a block's `attributes`, only client-side block-support JS does; every DesignSetGo extension attribute is a second, larger gap, since extensions register purely in JS). So PHP instead treats a name as known when it's in `WP_Block_Type->attributes` **or** in the committed manifest at `includes/abilities/agent-build/data/attribute-manifest.json` (`{blockName: [attributeName, ...]}`, generated by `npm run engine -- attribute-manifest --out includes/abilities/agent-build/data/attribute-manifest.json` — see `src/engine/node/attribute-manifest.js`) — unknown to *both* is the only case PHP rejects. A Jest test (`tests/unit/engine/attribute-manifest-freshness.test.js`) and a real-CLI test (`tests/engine/cli.test.mjs`) both regenerate the manifest and fail if the committed file is stale. `Attribute_Suggest` (`includes/abilities/agent-build/class-attribute-suggest.php`) is a PHP port of the same Damerau-Levenshtein suggestion logic, so the "did you mean" text matches whichever side catches the problem first.
