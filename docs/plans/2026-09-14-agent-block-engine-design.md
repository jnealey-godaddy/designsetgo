...
| `designsetgo_sanitized_content_changed` | report `invalid` entry | The submitter lacked `unfiltered_html`, and KSES-filtering the assembled markup left this block invalid or changed the block structure (a block's name or child count) |
...
1. **Attributes, at `build-page`** (`Tree_Kses`): each node's `attributes` go through core's `filter_block_kses_value( $attributes, 'post', wp_allowed_protocols(), array( 'blockName' => $name ) )` — the filter `filter_block_kses()` applies to a block comment's attributes, including attribute keys and the `core/template-part` `tagName` check. Like core, it entity-encodes plain text (a CSS `.a > .b` is stored as `.a &gt; .b`).
2. **Rendered markup, in the editor** (`Build_Sanitize_REST`): attribute filtering cannot see what `save()` renders — a `javascript:` URL in a URL attribute, an unsafe inline `style`. After `assemble` and before applying (in both the auto-save and review branches), the finish plugin POSTs the markup to `POST /designsetgo/v1/agent-build/{id}/sanitize` with `{ buildId, markup }` (`edit_post`; 404 missing post; 409 `designsetgo_build_mismatch`; 413 over 1 MB, checked before filtering). The route returns `{ markup: wp_kses( $markup, 'post' ) }`: core's `pre_kses` hook runs `filter_block_kses()` over the block comments, and HTML attributes get protocol and `safecss_filter_attr` filtering. The plugin parses the result and applies the **sanitized** blocks only if every block is valid and the structure (names and child counts, recursively) matches the unfiltered parse. Otherwise it reports `failed` with a `designsetgo_sanitized_content_changed` entry per changed block and applies nothing; a failed sanitize request is `failed` too. In practice a `core/button` `url: "javascript:alert(1)"` is applied as `href="alert(1)"` (the url re-parses from the filtered href, so the block stays valid), while a `designsetgo/icon-button` with the same url fails, because its `url` lives in the block comment and no longer matches the filtered href. Ordinary DSGo markup comes back byte-identical.
