...
- **Layout note**: While Align Rows is on, a card's content is placed on the shared grid rows, which **replaces the card's own layout**. A Section/Row card's **Justify Content / vertical alignment** and **Constrain Inner Width** no longer apply, and a **Group** card's own **Row/Grid layout** is overridden — its content is laid out as single-column rows (so don't use Align Rows on Group cards that rely on their own multi-column layout). Turn Align Rows off to restore the card's own layout.
