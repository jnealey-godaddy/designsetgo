...
- **Layout note**: While Align Rows is on, a Section/Row card lays its content out on the shared grid rows, so that card's own **Justify Content / vertical alignment** and **Constrain Inner Width** settings no longer apply (the row tracks position and full-bleed the content). Turn Align Rows off to go back to the card's own flex layout.
