...
class DesignSetGo_Progress_Bar_Stock_Binding_Test extends WP_UnitTestCase {
	 ...
	 * a `clamp(0%, calc(100% * var(--dsgo-progress, 75) / var(--dsgo-progress-max, 100)), 100%)`
	...
	private function progress_bar_html() {
		return '<div class="wp-block-designsetgo-progress-bar dsgo-progress-bar">'
			...
			. '<div class="dsgo-progress-bar__fill" style="width:clamp(0%, calc(100% * var(--dsgo-progress, 75) / var(--dsgo-progress-max, 100)), 100%);height:100%;transition:width 1.5s ease-out;border-radius:4px"></div>'
			. '</div></div>';
	}
	...
	public function test_stock_binding_resolves_inside_a_query_item_context() {
		...
		$this->assertStringContainsString(
			'width:clamp(0%, calc(100% * var(--dsgo-progress, 75) / var(--dsgo-progress-max, 100)), 100%)',
			$html
		);
	}
