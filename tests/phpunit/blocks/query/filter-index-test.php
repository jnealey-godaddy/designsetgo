<?php
/**
 * PHPUnit tests for the Dynamic Query filter index schema.
 *
 * @package DesignSetGo
 * @group query-block
 */
class DesignSetGo_Query_Filter_Index_Test extends WP_UnitTestCase {
	...
	public function test_reindex_post_writes_taxonomy_rows() {
		\DesignSetGo\Blocks\Query\FilterIndex::install();
		\DesignSetGo\Blocks\Query\FilterRegistry::register( 'category', array(
			'type'   => 'taxonomy',
			'source' => 'category',
		) );
		...
		\DesignSetGo\Blocks\Query\FilterIndex::reindex_object( 'post', $post_id );

		global $wpdb;
		$table  = $wpdb->prefix . 'dsgo_query_filter_index';
		$values = $wpdb->get_col( $wpdb->prepare(
			"SELECT filter_value FROM {$table} WHERE object_id = %d AND object_type = 'post' AND filter_key = 'category'",
			$post_id
		) );
	...
	public function test_reindex_post_writes_meta_rows() {
		\DesignSetGo\Blocks\Query\FilterIndex::install();
		\DesignSetGo\Blocks\Query\FilterRegistry::register( 'price', array(
			'type'   => 'meta',
			'source' => '_price',
		) );
		...
		$table  = $wpdb->prefix . 'dsgo_query_filter_index';
		$values = $wpdb->get_col( $wpdb->prepare(
			"SELECT filter_value FROM {$table} WHERE object_id = %d AND filter_key = 'price' ORDER BY filter_value",
			$post_id
		) );
	...
	public function test_reindex_is_idempotent() {
		...
		$count = (int) $wpdb->get_var( $wpdb->prepare(
			"SELECT COUNT(*) FROM {$table} WHERE object_id = %d AND filter_key = 'category'",
			$post_id
		) );
	...
	public function test_reindex_user_type_is_noop_in_a2() {
		global $wpdb;
		\DesignSetGo\Blocks\Query\FilterIndex::install();
		\DesignSetGo\Blocks\Query\FilterRegistry::register( 'role', array(
			'type'   => 'meta',
		...
		$wpdb->insert( $table, array(
			...
			'object_type' => 'post',
			'filter_key'   => 'sentinel',
			'filter_value' => 'survives',
		), array( '%d', '%s', '%s', '%s' ) );

		\DesignSetGo\Blocks\Query\FilterIndex::reindex_object( 'user', $user_id );
		...
		$surviving = (int) $wpdb->get_var( $wpdb->prepare(
			"SELECT COUNT(*) FROM {$table} WHERE object_id = %d AND object_type = 'post' AND filter_key = 'sentinel'",
			$user_id
		) );
	...
	public function test_reindex_skips_meta_values_exceeding_column_width() {
		global $wpdb;
		\DesignSetGo\Blocks\Query\FilterIndex::install();
		\DesignSetGo\Blocks\Query\FilterRegistry::register( 'url', array(
			'type'   => 'meta',
			'source' => '_canonical',
		) );
		...
		$table  = $wpdb->prefix . 'dsgo_query_filter_index';
		$values = $wpdb->get_col( $wpdb->prepare(
			"SELECT filter_value FROM {$table} WHERE object_id = %d AND filter_key = 'url'",
			$post_id
		) );
	...
	public function test_save_post_triggers_reindex() {
		...
		\DesignSetGo\Blocks\Query\FilterIndexHooks::register_hooks();
	...
	public function test_taxonomy_change_on_registered_filter_triggers_reindex() {
		\DesignSetGo\Blocks\Query\FilterIndex::install();
		\DesignSetGo\Blocks\Query\FilterRegistry::register( 'category', array(
			'type'   => 'taxonomy',
			'source' => 'category',
		...
		$table  = $wpdb->prefix . 'dsgo_query_filter_index';
		$values = $wpdb->get_col( $wpdb->prepare(
			"SELECT filter_value FROM {$table} WHERE object_id = %d AND filter_key = 'category'",
			$post_id
		) );
	...
	public function test_unregistered_taxonomy_change_does_not_reindex() {
		...
		// Register an unrelated taxonomy we're NOT tracking as a filter.
		...
		$this->assertSame( '1', $wpdb->get_var( $wpdb->prepare(
			"SELECT COUNT(*) FROM {$table} WHERE object_id = %d AND filter_value = 'sentinel'",
			$post_id
		) ) );
	}

	public function test_meta_change_on_registered_filter_triggers_reindex() {
		\DesignSetGo\Blocks\Query\FilterIndex::install();
		\DesignSetGo\Blocks\Query\FilterRegistry::register( 'price', array(
			'type'   => 'meta',
			'source' => '_price',
		...
		$table  = $wpdb->prefix . 'dsgo_query_filter_index';
		$values = $wpdb->get_col( $wpdb->prepare(
			"SELECT filter_value FROM {$table} WHERE object_id = %d AND filter_key = 'price'",
			$post_id
		) );
	...
	public function test_unregistered_meta_change_does_not_reindex() {
		...
		// Update a DIFFERENT meta key — not a registered filter.
	...
	public function test_reindex_object_removes_non_published_post() {
		...
		global $wpdb;
		$this->assertSame( 0, (int) $wpdb->get_var( $wpdb->prepare(
			'SELECT COUNT(*) FROM ' . \DesignSetGo\Blocks\Query\FilterIndex::table_name() . ' WHERE object_id = %d',
			$pid
	...
	// -------------------------------------------------------------------------
	// Codex MEDIUM #3 — server-side filter registration on save_post
	// -------------------------------------------------------------------------

	public function test_save_post_registers_filters_from_query_filter_blocks() {
		\DesignSetGo\Blocks\Query\FilterIndex::install();
		\DesignSetGo\Blocks\Query\FilterIndexHooks::register_hooks();
		...
		$filters = get_option( \DesignSetGo\Blocks\Query\FilterRegistry::OPTION, array() );
		$this->assertArrayHasKey( 'category', $filters, 'category filter must be registered after publishing a post with a query-filter block.' );
		$this->assertSame( 'taxonomy', $filters['category']['type'] );
	}

	public function test_save_post_does_not_register_filters_for_draft() {
		\DesignSetGo\Blocks\Query\FilterIndex::install();
		\DesignSetGo\Blocks\Query\FilterIndexHooks::register_hooks();
		...
		$filters = get_option( \DesignSetGo\Blocks\Query\FilterRegistry::OPTION, array() );
		$this->assertArrayNotHasKey( 'post_tag', $filters, 'Draft posts must not register filters.' );
	}
	...
	public function test_count_for_options_zero_fills_missing_values() {
		...
		$counts = \DesignSetGo\Blocks\Query\FilterIndex::count_for_options( 'category', array( $news, $orphan ), array() );

		$this->assertSame( 1, $counts[ (string) $news ] );
		$this->assertSame( 0, $counts[ (string) $orphan ] );
	}

	public function test_count_for_options_respects_cross_filter_intersection() {
		// 3 posts in news; 2 of them tagged hot; count for news under active tag=hot should be 2.
		\DesignSetGo\Blocks\Query\FilterIndex::install();
		\DesignSetGo\Blocks\Query\FilterRegistry::register( 'category', array( 'type' => 'taxonomy', 'source' => 'category' ) );
		\DesignSetGo\Blocks\Query\FilterRegistry::register( 'tag',      array( 'type' => 'taxonomy', 'source' => 'post_tag' ) );
	...
	public function test_count_for_options_excludes_self_filter_from_intersection() {
	...
	public function test_count_for_options_empty_option_values_returns_empty_array() {
		\DesignSetGo\Blocks\Query\FilterIndex::install();
		$this->assertSame( array(), \DesignSetGo\Blocks\Query\FilterIndex::count_for_options( 'anything', array(), array() ) );
	}
	...
	public function test_count_for_options_deduplicates_option_values() {
		...
		$counts = \DesignSetGo\Blocks\Query\FilterIndex::count_for_options( 'category', array( $news, $news, (string) $news ), array() );
		$this->assertCount( 1, $counts );
		$this->assertSame( 1, $counts[ (string) $news ] );
	}

	public function test_is_available_true_for_registered_filter() {
		\DesignSetGo\Blocks\Query\FilterRegistry::register( 'category', array( 'type' => 'taxonomy', 'source' => 'category' ) );
		$this->assertTrue( \DesignSetGo\Blocks\Query\FilterIndex::is_available( 'category' ) );
	}

	public function test_is_available_false_for_unregistered_filter() {
		$this->assertFalse( \DesignSetGo\Blocks\Query\FilterIndex::is_available( 'nonexistent' ) );
	}

	public function test_count_for_options_self_exclusion_uses_sanitized_key() {
		// Regression: $active_filters lookup must use the sanitized key form, not raw $filter_key.
		\DesignSetGo\Blocks\Query\FilterIndex::install();
		\DesignSetGo\Blocks\Query\FilterRegistry::register( 'category', array( 'type' => 'taxonomy', 'source' => 'category' ) );
