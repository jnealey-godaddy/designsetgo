...
- **Icon blocks send only the icons a page uses.** Pages with an Icon Button, Icon List, Tabs, or Modal Trigger used to download all ~340 icons (about 51 KB) inline. Now they get just the icons that appear on the page. Icons added later without a page reload, such as Query "load more" results, are fetched as needed from a cached `designsetgo/v1/icons` route. Also fixed the icon alias map, which was never loading on the server because of a wrong file path.
- **Extension scripts and styles load per feature.** Animations, background video, clickable groups, text reveal, expanding backgrounds, parallax, hover effects, reveal, SVG patterns, and interactions were one bundle, sent to any page with DesignSetGo content. Each is now its own file, loaded only when a block on the page uses it. The editor-only block placeholder styles no longer ship to visitors.
- **Blocks and extensions work in content that arrives without a page load.** After Airo's live refresh or a Query "load more", any DesignSetGo block or extension that wasn't on the original page now loads its scripts and styles, including the settings and translations those scripts need. Before, a Tabs, Accordion, Slider or Form block that first appeared that way stayed inert until the page was reloaded. After a live refresh this applies to people who can edit posts (Airo's editing session). Other soft-navigation integrations can turn it on with the `designsetgo_frontend_asset_manifest` filter. For "load more", a Query now loads the scripts and styles of the blocks in its item template when the page renders, even if the first page of results is empty. The Query block's own script can't be added after a page loads, so it still needs a full page load.
- **Dynamic Image serves responsive images.** Attachment images now get `srcset` and `sizes`, so the browser picks a size that fits instead of always downloading the full-size file. WordPress now chooses lazy or eager loading, so a Dynamic Image used as the hero is no longer lazy-loaded.
- **Hotspot and Card images get dimensions and `srcset`.** These are added when the page is rendered, so there's no block markup change and no migration. This stops layout shift and lets WordPress lazy-load the images. A Card's background-layout image is now a lazy-loaded image instead of a CSS background, which always downloaded, even far below the fold.
...
- **Select fields use a translated default without saving the editor's language into the page.** An unset placeholder now follows the site language on the frontend and the user's language in the editor. Reset and Reset all restore that unset state; custom text is preserved, and clearing the field explicitly removes the placeholder. Older static select fields also preserve their text or empty-placeholder choice when migrating to the dynamic block.
- **Bundled translations fill gaps in installed language packs.** Older WordPress language packs could keep the select-field placeholder in English even on a French site. DesignSetGo now uses its bundled catalogs for missing PHP and editor JavaScript translations while preserving existing language-pack translations, including after locale switches. No language-pack files need to be removed or replaced.
- **Card blocks stay valid when reopened in another editor language.** Saved badge accessibility labels and decorative-image fallback text are preserved instead of being translated again during validation. This also applies to older Card markup, while authored image alternative text remains editable.
- **Older Map blocks migrate across editor languages without "Attempt Recovery."** Validation reads the saved map labels, load-button text, and privacy notice from the existing markup. After migration, the dynamic block renders its default interface text in the site language and retains any authored privacy notice.
- **Block exclusions reflect settings changes within the same request.** Extension and animation-default checks now read the refreshed settings cache, so adding or removing an exclusion no longer uses an outdated list captured during block registration.
- **Form Builder: forms that share a form ID were validated and emailed with another copy's settings.** DSGo patterns ship fixed IDs (`contact-professional` is in two), and duplicating a block copies its ID, so one site could hold several published forms under one ID. The handler took whichever the database returned first, so a submission could fail on another copy's required fields, or send its notification to another copy's recipient — reported as "form submits, no email arrives". Submissions now carry the page they came from (`sourcePostId`, falling back to the referer), and a published copy on that page wins; the notification reuses that resolved form instead of looking it up again. In the editor, a form whose ID is new to the post (pattern insert, paste) or already used by an earlier form gets a fresh ID on mount. Forms inside synced patterns, template parts, and a template shown around the post are left alone.
- **Section shape dividers showed a faint line across the section at some browser zoom levels.** The line sat at the divider's inner edge, in the band colour (white by default), and came and went as the page was zoomed. Chrome snaps the mask to device pixels but still painted the band into the partial pixel row at the `::before`'s section-facing edge, where the exclude mask has its full-cover layer and no shape coverage. The band is now painted as a top-anchored gradient that stops 2px short of that edge, where every exclude-mask shape is solid, so nothing visible changes. Tonal shapes use a single-layer mask, never had the line, and still paint the full box. CSS only, no markup change.

### Internal

- **Added translation and migration regression coverage** for select placeholders, Card and Map compatibility, differing site and editor languages, and incomplete installed PHP and JavaScript language packs. Documented the select-field validation workflow and language-pack fallback checks.
- **Stabilized pattern E2E tests.** Pattern lookup and insertion use separate browser evaluations to avoid the collected-Promise error reported as "Execution context destroyed," with checks against duplicate insertion. Frontend review opens a separate tab so pending editor REST requests can finish, while errors from both pages remain checked. Test setup saves and restores the administrator's language, and desktop visibility assertions set their viewport explicitly even in mobile profiles.
- **Fixed test setup and tooling.** Added the missing `@testing-library/dom` dependency, removed an obsolete ESLint rule suppression, corrected npm argument forwarding in the documented wp-env PHPUnit commands and bootstrap instructions, and replaced the skipped block-exclusion test with assertions covering exclusion changes.

## [2.7.5] - 2026-09-10

### Fixed

- **Dynamic Query's filter index table failed to install on MySQL servers with a 1000-byte index limit, and the failure slowed every wp-admin request (#551).** The two composite keys indexed full `VARCHAR(190)` columns. Under utf8mb4 that is 1520 and 1680 bytes, over the limit a MyISAM default storage engine enforces, so `CREATE TABLE` failed with error 1071. The keys now index the first 80 characters of `filter_key` and `filter_value`, which brings them to 644 and 805 bytes. Filter lookups are equality and `IN` matches, which a prefix index still serves. Tables that already installed are left alone, because dbDelta compares indexes without their prefix lengths.

  The upgrade also stopped retrying on every admin request. The version gate only sealed once the table existed, so a rejected schema re-ran `dbDelta` on each `admin_init`, logged the same error each time, and was enough to push a core update past its timeout and strand the update lock. A failure is now recorded with its database error and retried after a day, straight away after a plugin update, or from a **Retry now** link. Administrators see an error notice naming the database error instead of a silent log entry. Each administrator can dismiss it, which hides it everywhere except DesignSetGo's own admin pages, where it stays until the table exists. A different error, or a plugin update that still fails, brings it back everywhere. `wp dsgo query index drop` clears the recorded failure along with the version.
- Shape dividers, SVG patterns, expanding backgrounds and text reveal effects now work on blocks added by an AI assistant. The editor used to flag those blocks as invalid.

## [2.7.4] - 2026-09-10

### New Features

- AI and agent tooling — a `designsetgo/get-design-context` ability returns the theme's resolved settings, global styles and block style variations, so an assistant can build with the site's own palette, fonts and spacing instead of guessing. It is read-only.

### Improved

- Form Builder — one visitor's IP address can now send at most 20 submissions a minute across all forms on the site, on top of each form's own limit. Adjust it with the `designsetgo_form_global_rate_limit_count` and `designsetgo_form_global_rate_limit_window` filters.
- Pages that don't use Grid, Row, Icon or Pill no longer carry those blocks' CSS inline in the page head.

### Security

- **Dynamic Query refresh renders only what the site rendered.** The public `designsetgo/v1/query/render` route no longer takes `attributes` / `innerBlocks` from the caller. First paint embeds a signed refresh source in `[data-dsgo-blobs-for]` (`data-dsgo-refresh-source` + `data-dsgo-signature`, an HMAC bound to the site), and the route renders only a definition whose signature verifies — so it works wherever the query is placed (post content, templates, template parts, synced patterns, widgets, nested loops) without trusting the request. A query inside a post's content is limited to people who can see that post. Direct callers must now send `source` + `signature` instead of `attributes` + `innerBlocks`.
- **Editor preview needs edit rights.** `designsetgo/v1/query/render-preview` requires `edit_posts` (was `read`) and only renders post types the user can see or edit; its output never carries a signed source.
- **User email is private in user queries.** Searching or sorting a users query by email now needs `list_users`, matching core's users endpoint.
- **Visitors get no REST nonce** in the query region's context. The public route needs none, and a stale one on a cached page is rejected by core before the route runs.
...
- **Protected query content stays private.** Grouping by a protected custom field (one whose name starts with an underscore) no longer exposes its values as headings, and queries on password-protected pages remain locked until the password is entered.

### Fixed

- **Form Builder**: a default Phone field's country-code select and Cloudflare Turnstile's injected `cf-turnstile-response` input no longer fail submission. The server declares `{field}_country_code` and drops (rather than rejects) any name the saved form doesn't declare. Forms in block widgets, theme-file templates and registered patterns are recognised again.
- Dynamic Query — filters, sort, search, Load more and infinite scroll now work for visitors who aren't logged in. On 2.7.3 the request behind them required a login, so for everyone else clicking did nothing.
- Dynamic Query — Load more keeps the active filters, search and sort. It used to fetch the next page of the unfiltered results and add it under the filtered ones.
- Dynamic Query — accordions, counters, flip cards, maps, modals and other interactive blocks inside results added by Load more or infinite scroll now start up. Only the first page's items used to work.
- Dynamic Query filter counts update straight away when a post loses the last category, tag or field value a filter counts. The cached count used to keep including it.
- Tabs — a Tabs block placed inside another one keeps its own panels. The outer block counted the inner block's panels as its own and showed or hid them as you switched tabs.
- Scroll Accordion — an accordion removed from the page, for example when filters replace a Dynamic Query's results, stops listening for scroll and resize. It kept measuring elements that were no longer there on every scroll.
- Map — an address containing quotation marks no longer sends the map to New York. The quotes reached the geocoder as stray `u0022` text, so the lookup failed and the map fell back to its default location.
- More blocks added by an AI assistant now come out valid. A Section that isn't width-constrained no longer gets an inner max-width, Row and Grid keep their own padding and gap (including zero) instead of reverting to the defaults, Row honours vertical alignment including space-between, Grid honours a minimum column width and matched row heights, and Card keeps its border colour.

## [2.7.3] - 2026-09-04

### Security

- Form Builder no longer writes email notification settings — recipient, subject, from and reply-to addresses — into the page's public markup.

### Fixed

- Blocks added by an AI assistant now come out valid. When a block was built through the plugin's Abilities API rather than typed into the editor, the markup it produced did not always match what the editor writes, so opening the page showed "Attempt Recovery" or silently rewrote content the author never touched. Every block is now checked against its own editor output, including at its plain defaults.
- Section, Row and Grid keep their hover and overlay colours when built this way — the five custom properties behind them were dropped entirely.
- Modal now uses the label you gave it for screen readers instead of always announcing "Modal".
- Icon Button and Modal Trigger apply padding, colour, typography, border and shadow to the button itself rather than duplicating it on the wrapper.
- Counter Group honours its own column counts, Grid honours its HTML element setting, and Tabs keeps its colour settings.
- Alignment set on a block built this way is now carried into the markup, for blocks that align left/centre/right as well as those offering wide and full.

## [2.7.2] - 2026-08-26

### Fixed

- Advanced Heading — the typing effect no longer hides the last letter of each word. It also now types one character at a time, so short words no longer appear in twelve tiny slivers.
- Advanced Heading — the typing, clip, and blinds effects no longer shave the tops and tails off letters. Descenders on g, j, p, q, y and accented capitals were being cut.
- Text Path — turning on Motion now animates in the editor, not only on the published page. The frontend was always animating; the editor just never previewed it, which made the setting look broken.
- Advanced Heading — segments no longer run together in the editor. Plain segments were missing the spacing the published page gets, so the editor showed them squished while the frontend looked correct.
...
- The Chart block works again. On 2.7.0, inserting a Chart showed "Error loading block" instead of the chart — the block's preview is drawn on the server, and the request that draws it was being rejected. Charts already saved on a page were unaffected; only the editor preview failed.
- Chart y-axis labels stay inside the chart. Longer labels — anything past about seven characters, which includes any formatted currency — were painted outside the block's own area, over whatever sat to the left of them. The space reserved for them is now measured from the labels themselves.
...
- Hotspot — place interactive markers over an image, each with its own tooltip. Fully keyboard accessible.
- Text Path — flow a line of text along a wave, arc, circle, oval, spiral, straight line, or an SVG path of your own, with controls for arc size, rotation, word spacing, and padding, plus an optional motion mode that travels the text along the path.
- Animated Headline — Advanced Heading gains two headline modes. Rotating cycles a list of phrases through one of nine effects (typing, clip, flip, swirl, blinds, drop-in, wave, slide, slide-down), forward or in reverse, with duration, delay, and loop controls. Highlighted draws a hand-drawn mark behind or through a phrase — circle, curly, underline, double underline, zigzag, diagonal, strikethrough, or an X.
...
- Schema.org markup — opt an Accordion into FAQ or How-to structured data, and a Star Rating into Review or Aggregate rating. Nothing is emitted unless you deliberately choose a type, and password-protected content is never exposed.
- Off-canvas panel — Modal gains a panel mode that slides in from any edge, available as its own block variation, with a size control. It keeps the modal's focus trap, Escape handling, scroll lock, and triggers.
...
- Map — a keyless Google Maps option that needs no API key at all, and loads no JavaScript on the page. The existing keyed Google and OpenStreetMap options are unchanged, and privacy mode still holds the map back until the visitor clicks Load Map.

### Improved
- **`designsetgo/add-blocks` inserts a page's sections in one call.** Staging a generated page used to cost one `add-block` request per top-level section, run one after another because each rewrites the same post. The batch ability takes the same `block_name` / `attributes` / `inner_blocks` definitions as `add-block`, writes the post once, and produces byte-identical content to the equivalent sequence of `add-block` calls. It is all-or-nothing: every entry is screened before anything is saved, and a refusal names the entry (`block_index`, `blocks[n]: …`) as data rather than a `WP_Error`, so the index survives the MCP bridge. `add-block` now shares the same placement, sanitization and write path.
...
### Fixed
- **A theme or style kit's own decorative overlay on a Section with a background was invisible until the section was hovered.** The Section block's hover-background layer is a `::after` pseudo-element that is always present and held at `opacity: 0` until `:hover`, regardless of whether a hover color was actually configured. With no gate, that unconditional `opacity: 0` was the only declaration touching the `opacity` property on `.wp-block-designsetgo-section.has-background::after`, so any kit painting its own overlay there — via `background` alone, with no `opacity` of its own — inherited DesignSetGo's `opacity: 0` and only appeared once hovered, no matter how specific the kit's selector was. Both the hover layer and its `:hover` fade-in are now gated on the `--dsgo-hover-bg-color` inline variable actually being present (`&[style*="--dsgo-hover-bg-color"]`), matching the gate the Row and Grid blocks already use for the same variable, so a section with no hover background configured leaves `::after` completely unclaimed for a kit to paint.
...
- Advanced Heading and Hotspot editing is smoother on long pages, duplicating a Text Path no longer breaks the original's shape, and hotspot tooltips sit on their markers instead of drifting away from them.
- Translations — removed a JavaScript translation catalog whose files were keyed so that WordPress could never load them, and regenerated the translation template.

## [2.6.3] - 2026-08-11

### Fixed

- Grid columns with a minimum width set no longer run past the edge of the page. On a theme with a narrower content width, a grid asking for more columns than could fit at that minimum pushed its columns straight out of the content area. The grid now drops to fewer columns and wraps the extra items onto the next row, and still shows your chosen number of columns whenever there's room for them. Existing grids are unchanged.
- The Overlay Header panel no longer causes an error when editing a template part in the Site Editor. The panel now appears only where the setting actually applies.
