...
### New Features
- **Theme animation defaults**: set an entrance animation per block type (e.g. all Buttons fade in) once, in Settings → DesignSetGo → Animations or in theme.json (`settings.custom.designsetgo.blockAnimations`). Every block of that type inherits it automatically; individual blocks can override (Custom) or opt out (Off). One rule can target several block types at once — search and pick them from the block list, or choose a `namespace/*` wildcard.
