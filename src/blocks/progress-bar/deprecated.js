 ...
 * var(--dsgo-progress-max, 100)), 100%)` so a `dsgoStyleBinding` on
 * `--dsgo-progress` can drive the fill from the frontend (e.g. a stock bar
 * bound to `designsetgo/woo-stock-quantity`). Attribute schema is unchanged —
 * only the emitted markup differs — so this is a pure save() reproduction
 * with no isEligible/migrate.
 */
const v2 = {
	apiVersion: 3,
	attributes: metadata.attributes,
