...
const v3 = {
	...
	isEligible(attributes, innerBlocks, { innerHTML }) {
		// Old extension markup: a blobs wrapper carrying dsgo-has-max-width with
		// a RAW inline max-width. `(?:^|[^-])max-width:` excludes the new
		// --dsgo-blob-max-width var (preceded by `-`), so current native
		// content is never flagged.
		return (
			!!innerHTML &&
			innerHTML.includes('dsgo-blobs-wrapper') &&
			innerHTML.includes('dsgo-has-max-width') &&
			/(?:^|[^-])max-width:/.test(innerHTML)
		);
	},
	save({ attributes }) {
		const {
