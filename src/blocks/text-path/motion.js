...
/**
 * Text Path motion engine.
 *
 * Shared by the frontend view script and the editor, so the canvas previews
 * the motion an author just switched on. The editor imports this module
 * directly rather than view.js, which carries the frontend bootstrap.
 *
 * @package
 */
...
function getOpenPathMaximumOffset(textPath) {
	...
	// Resolve against the textPath's own document, not this module's. In the
	// editor the block lives inside the canvas iframe, so a bare `document`
	// lookup searches the parent frame and never finds the guide.
	const scope = textPath.ownerDocument || document;
	const guide = guideId ? scope.getElementById(guideId) : null;
...
/**
 * Stop and forget the motion running on a block, restoring its start offset.
 *
 * The editor needs this: toggling Motion off, or unmounting the block, has to
 * cancel the frame loop, which would otherwise keep running against a
 * detached element.
 *
 * @param {Element|Document} root Element to stop, or a root to search.
 */
export function stopTextPathMotion(root = document) {
	const blocks = [];

	if (root.matches?.(MOTION_SELECTOR) || motionStates.has(root)) {
		blocks.push(root);
	}
	...
	blocks.forEach((block) => {
		...
		pause(state);
		state.textPath.setAttribute('startOffset', `${state.initialOffset}%`);
		motionStates.delete(block);
	});
}
