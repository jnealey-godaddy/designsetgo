...
export default function QuerySourcePanel({ attributes, setAttributes, clientId }) {
	const {
		source, postType, perPage, offset, orderBy, orderByMetaKey, order,
		columns, columnsTablet, columnsMobile, columnGap,
	} = attributes;
	...
	return (
		...
		>
			...
			>
				<RangeControl
					label={ __( 'Columns', 'designsetgo' ) }
					help={ __( 'Desktop columns. Tablet and mobile narrow further by default.', 'designsetgo' ) }
					value={ columns }
					min={ 1 }
					max={ 6 }
			...
			>
				<RangeControl
					...
					min={ 0 }
					max={ 6 }
					onChange={ ( value ) => setAttributes( { columnsTablet: value || 0 } ) }
					__next40pxDefaultSize
					__nextHasNoMarginBottom
				/>
			</DsgoInspectorPanel.Item>
			...
			>
				<RangeControl
					...
					min={ 0 }
					max={ 3 }
					onChange={ ( value ) => setAttributes( { columnsMobile: value || 0 } ) }
					__next40pxDefaultSize
					__nextHasNoMarginBottom
				/>
			</DsgoInspectorPanel.Item>
			...
			>
				<TextControl
					label={ __( 'Column gap', 'designsetgo' ) }
					help={ __( 'CSS length (e.g. 1.5rem, 24px). Leave blank for default.', 'designsetgo' ) }
					value={ columnGap }
					onChange={ ( value ) => setAttributes( { columnGap: String( value || '' ) } ) }
					__next40pxDefaultSize
					__nextHasNoMarginBottom
				/>
			</DsgoInspectorPanel.Item>
			...
			{ showMetaKey && (
				...
				>
					<TextControl
						...
						onChange={ ( value ) => setAttributes( { orderByMetaKey: String( value || '' ) } ) }
						__next40pxDefaultSize
						__nextHasNoMarginBottom
					/>
				</DsgoInspectorPanel.Item>
