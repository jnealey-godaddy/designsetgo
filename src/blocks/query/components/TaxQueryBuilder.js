...
export default function TaxQueryBuilder( { attributes, setAttributes, clientId } ) {
				...
				<VStack spacing={ 3 }>
					{ taxQuery.clauses.length > 1 && (
						<SelectControl
							...
							onChange={ ( v ) =>
								setAttributes( { taxQuery: { ...taxQuery, relation: v } } )
							}
					...
					{ taxQuery.clauses.map( ( clause, idx ) => (
						<VStack key={ idx } spacing={ 2 } className="dsgo-query-tax-clause">
							<SelectControl
								...
								onChange={ ( v ) =>
									updateClause( idx, { taxonomy: v, terms: [] } )
								}
								__next40pxDefaultSize
								__nextHasNoMarginBottom
							/>
							...
							<HStack>
								...
								<Button
									...
									aria-label={ __( 'Remove taxonomy filter', 'designsetgo' ) }
								>
