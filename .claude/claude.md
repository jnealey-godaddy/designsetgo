...
- URL params: `q`, `sort`, `filter_<taxonomy>`. Extend via `designsetgo_query_url_params` filter. Scoped per Query (v2.6): `{key}__{queryId}` wins over a same-named bare key, and a key scoped for a different query is never read; a bare key is still honored by every Query (unchanged pre-2.6 contract, incl. WooCommerce's own filter params) — see "Query block family (Dynamic Query v2.6)" below.
...
- URL params are now scoped per Query, fixing a cross-query leak: an INTERACTION on one `designsetgo/query` block's own filter control (checking a checkbox, picking a select option) used to also silently filter an unrelated `designsetgo/query` on the same page, even one with no matching filter control.
- `designsetgo_query_extract_params_from_request( $query_id )` (`render-helpers.php`) prefers a query-scoped key (`{key}__{queryId}`) over a same-named bare key, and never reads a key scoped for a different query. A bare/legacy key (`?filter_category=news` with no `__{queryId}` suffix) is honored by **every** query, unchanged from before this fix — that's the pre-existing, intentional contract behind a menu link or widget to a filtered listing. `min_price`/`max_price`/`rating_filter` are likewise never scoped — Woo's filter blocks can't emit a `queryId`.
- An earlier version of this fix additionally gated the bare-key fallback by whether the query "declared" a matching filter block (`designsetgo_query_collect_declared_params()`). That broke the bare-key contract above and was **reverted** (`DesignSetGo_Query_Filter_Server_Test` failures, PR #592) — there is no declared-params gate. Scoping alone (not declaration) is what stops one query's filter *interaction* from leaking into another's.
- `query-filter/render.php` renders every form field's `name` attribute already scoped (`filter_category__{queryId}`) — both the IAPI actions (which read `paramName` off the DOM `name`) and a no-JS `<form method="get">` submission write the scoped key with no further code needing to know about scoping, so an actual interaction never produces a bare key. `designsetgo_query_filter_get_scoped_value()` / `designsetgo_query_filter_owned_get_params()` are the read-side counterparts (active/reset chips, cross-filter counts) — a bare key is "owned" by every query there too, matching the server-side WP_Query behavior.
- Backward compatible: every existing bookmarked/shared `?filter_category=`, `?q=`, `?sort=` URL keeps working unchanged, on every Query on the page, exactly as before this task.
