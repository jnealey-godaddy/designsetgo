...
- **Tree contract (v1)**: `{ version: 1, blocks: [ { name, attributes?, innerBlocks? } ] }` — same shape as a WordPress block object. Any other `version` is `designsetgo_unsupported_tree_version`. Locations report as paths, e.g. `blocks[0].innerBlocks[2]`.
...
- **Remote build/finish flow**: `designsetgo/build-page` (`includes/abilities/agent-build/class-build-page.php`) validates tree *structure* only (PHP cannot run `save()`) and stores it as a pending build — `_dsgo_pending_tree` post meta holds `{ tree, mode, base }` as one JSON blob (`base` = `post_modified_gmt` at store time; there is **no separate `_dsgo_pending_base` key**, unlike the original spec). `new.post_type` is restricted to `show_in_rest` types that support `editor` and aren't `wp_*`-prefixed. The editor plugin (`src/engine/browser/finish/`) finishes it on load and reports one of: `pending → conflict | failed | finished | finished_with_findings | awaiting_review → finished/finished_with_findings | discarded`. A published post always goes to `awaiting_review` (applied unsaved, with a Discard notice) — it is never auto-saved. `designsetgo/get-build-status` polls `_dsgo_build_report`.
...
- **Known issue (parked)**: `designsetgo/section` with an explicit `style: {}` attribute assembles invalid (serialize vs. reparse differ) — the engine reports it `failed` rather than writing bad content. Fix belongs in `src/blocks/section`, tracked as a follow-up, not part of this engine.
