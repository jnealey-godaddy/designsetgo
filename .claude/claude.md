...
- These exist for what Woo exposes through no block: **raw scalars for `dsgoStyleBinding`**. A stock bar is `progress-bar` + `--dsgo-progress` bound to `woo-stock-quantity` — the fill reads `--dsgo-progress` as a raw count (not a percentage) against `--dsgo-progress-max` (default `100`; set it to a low-stock threshold via a second binding or a theme rule to scale the bar), and `showPercentage`'s label cannot follow the bound value, so turn it off on a bound bar.
