...
- URL params: `q`, `sort`, `filter_<taxonomy>`. Extend via `designsetgo_query_url_params` filter. Scoped per Query (v2.6): `{key}__{queryId}` wins over a same-named bare key; a bare key is honored only when the Query declares it (own filter blocks' `paramName`, or `bindSearchTo`) — see "Query block family (Dynamic Query v2.6)" below. WooCommerce's own filter params stay unscoped by design.
...
### Query block family (Dynamic Query v2.6)

- URL params are now scoped per Query, fixing a cross-query leak: two `designsetgo/query` blocks on one page used to filter each other via a shared `filter_<taxonomy>`/`q`/`sort` key, even when only one of them had a matching filter control.
- `designsetgo_query_extract_params_from_request( $query_id )` (`render-helpers.php`) prefers a query-scoped key (`{key}__{queryId}`) over a same-named bare key, and never reads a key scoped for a different query. `min_price`/`max_price`/`rating_filter` stay unscoped — Woo's filter blocks can't emit a `queryId`.
- `designsetgo_query_collect_declared_params( $blocks, $bindSearchTo )` walks a Query's own children (stopping at a nested `designsetgo/query` or an item-host's innerBlocks) to collect the bare param names its own `designsetgo/query-filter` children declare via `paramName`, plus its own `bindSearchTo` attribute. `render-posts.php`'s generic `filter_<taxonomy>` loop, bare-`q`, and `sort` handling all gate on this list — a query with no matching filter control ignores a bare/legacy param instead of silently applying it. WooCommerce's `filter_<attr>` attribute params are never gated this way (they fail `taxonomy_exists()` on their bare, `pa_`-stripped slug, so they're left to `render-woo.php` regardless).
- New context key `designsetgo/queryDeclaredParams` (raw array, not attribute-backed — injected directly into `$shared_context` the same way `designsetgo/queryTaxQuery` is) lets `query-filter`'s active/reset variations, and its cross-filter count queries, scope which `$_GET` keys are "theirs" instead of scanning every `filter_*`/`q`/`sort` key on the page.
- `query-filter/render.php` renders every form field's `name` attribute already scoped (`filter_category__{queryId}`) — both the IAPI actions (which read `paramName` off the DOM `name`) and a no-JS `<form method="get">` submission write the scoped key with no further code needing to know about scoping. `designsetgo_query_filter_get_scoped_value()` / `designsetgo_query_filter_owned_get_params()` are the read-side counterparts.
- Backward compatible: a single Query's existing bookmarked/shared `?filter_category=`, `?q=`, `?sort=` URLs keep working unchanged (the query declares those keys, so the bare fallback still applies).
